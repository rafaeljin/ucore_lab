...
alloc_proc(void) {
     ...
     */
     //LAB5 2012080059 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2012080059 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2012080059 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2012080059
    //LAB8:EXERCISE2 2012080059  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    if (copy_files(clone_flags, proc))  // LAB 8
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2012080059  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    char *uarg_buf = (USTACKTOP - uargv_size);
    char **uargv = (char **)uarg_buf - argc;
    for (i = 0; i < argc; i++) {
        uint32_t argvlen = strnlen(kargv[i], EXEC_MAX_ARG_LEN);
        memcpy(uarg_buf, kargv[i], argvlen);
        uarg_buf[argvlen] = '\0';
        uargv[i] = uarg_buf;
        uarg_buf += argvlen + 1;
    }
    ...
    *(int *)stacktop = argc;
     ...
     */
    tf -> tf_cs = USER_CS;
    tf -> tf_es = tf -> tf_ss = tf -> tf_ds = USER_DS;
	tf -> tf_esp = stacktop; // USTACKTOP
